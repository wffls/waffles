...
function nginx.upstream {
  waffles.subtitle "nginx.upstream"
  ...
  waffles.options.create_option options
  ...
  waffles.resource.process "nginx.upstream" "$_name"
}
...
function nginx.upstream.read {
  ...
  waffles_resource_current_state=$(augeas.get --lens Nginx --file "$_file" --path "/upstream/#name[. = '${options[name]}']")
...
function nginx.upstream.create {
  ...
  if [[ $_result =~ ^error ]]; then
    log.error "Error adding nginx.upstream $_name with augeas: $_result"
    return
  fi
}
...
function nginx.upstream.delete {
  ...
  if [[ $_result =~ ^error ]]; then
    log.error "Error deleting nginx.upstream $_name with augeas: $_result"
    return
  fi
}
