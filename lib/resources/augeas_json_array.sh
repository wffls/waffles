...
function augeas.json_array {
  waffles.subtitle "augeas.json_array"
  ...
  waffles.options.create_mv_option value
  ...
  waffles.resource.process "augeas.json_array" "$_name"
}
...
function augeas.json_array.read {
  ...
  waffles_resource_current_state=$(augeas.get --lens Json --file "${options[file]}" --path "${options[path]}")
